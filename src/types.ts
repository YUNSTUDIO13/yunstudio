...
// ============================================================
// 旅行（Travel）—— 个人旅行记录 / 旅行地图
// 数据：本地 Dexie 优先（与 movies/books 一致）；本模块暂未接 Supabase 云端表，
//       仅本地持久化（local-first），后续如需多端同步再按既有权限流程加 travels 表 + outbox。
// 字段：出发地 / 目的地 / 交通 / 住宿 / 地点 / 图片 / 定位 + 评分 + 日期 + 省份分组
// ============================================================
export type TransportMode = 'flight' | 'train' | 'car' | 'bus' | 'other'

export interface TravelStop {
  id: string
  place: string // 地点（景点 / 打卡点）
  accommodation?: string // 住宿
  transport?: TransportMode // 交通方式
  image?: string // 图片（URL / dataURL）
  location?: string // 定位（文本 / 经纬度）
}
...
export interface Travel {
  ...
  from: string // 出发地
  to: string // 目的地（城市）
  province: string // 所属省份（用于分组，如「湖南省」）
  transport: TransportMode // 主要交通方式
  accommodation?: string // 住宿
  place?: string // 主要地点 / 景点
  image?: string // 封面图片（URL / dataURL）
  location?: string // 定位（文本 / 经纬度）
  date: string // 旅行日期（YYYY-MM-DD）
  rating: number // 评分 0–10
  note?: string // 备注 / 游记
  stops?: TravelStop[] // 行程节点（支持按住滑动排序）
  order: number // 排序权重（面板内拖动排序用）
  created_at: string
  updated_at: string
}
